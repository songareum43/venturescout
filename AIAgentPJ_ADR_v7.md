...
- **통합: 4개 트랙(A·B·C·D) 머지 → `integration` 브랜치 + 실 RDS E2E** (ADR-033) — 계약 통일(contracts.py 3브랜치 동일) 후 D에서 분기해 A→ko-agent→track-b 머지(소유권 규칙으로 충돌 해소). **실 RDS 임베딩 데이터로 `/analyze` E2E 성공** — 하이브리드 검색→6 agent_runs(각 근거 5개)→`more_research` 도출, RDS 테스트행 CASCADE 정리. `pytest` 16 passed/2 failed(test_mock_graph — mock↔실 미스매치). `origin/integration` push, **main 미반영**.
- **live+Bedrock 풀 E2E 동작** (ADR-034~037) — 검색 2단계 최적화(풀스캔→인덱스 후보생성)+근거0건 graceful, HNSW 재생성, Bedrock 실연결(jp.sonnet-4-6), 임베더 이미지 캐시. 실 RDS 검색→실 Claude 분석→DB 적재→Evidence Board까지. C의 mock 전면제거(ADR-037)로 `RETRIEVAL=live` 전용.
- **Evidence Board 문구 개선** (ADR-038) — 판정 4종 설명·근거 한글 출처·반론 구체화·신호 한글 통일·다음실험 안내.
- **평가 하네스 N회 분포 승격** (ADR-035, ADR-030 실행).
- 검증: live `/analyze` E2E + `pytest` 30 passed(test_live_graph는 monkeypatch로 RDS 없이 통과)
...
### ADR-033 — (구현) 4개 트랙 통합 머지(`integration` 브랜치) + 실 RDS E2E
- **상태**: done (`origin/integration` push 완료, **main 미반영** — 오류 정리 후 PR 예정). ADR-032의 D 적응 + B의 계약 반영(track-b `ae9b7ec`·`35f44cd`)으로 계약이 3브랜치 통일된 뒤 첫 통합.
- **맥락**: 계약 분기 해소됨 — `shared/contracts.py`가 **D=ko-agent=track-b 바이트 동일**(B가 EvidenceItem 필수화·AgentFinding 분리 반영). A(데이터)·B(검색·임베딩+②③)·C(에이전트 graph·척추)·D(api/ui/harness·job_id·RDS)를 한 베이스로 합쳐 실데이터 E2E를 볼 시점. main은 보호 브랜치(ADR-021)라 직접 안 올림.
- **결정**: **D에서 `integration` 브랜치 분기 → A → ko-agent → track-b 순 머지.** 충돌 해소 소유권 규칙:
    * `agents/graph.py`·`shared/state.py`·`shared/contracts.py`·`tests/test_contracts.py` → **C(ko-agent)** (척추·계약 소유)
    * `retrieval/tools.py` → **B(track-b)** (검색 소유)
    * `db/init.sql`·`db/schema.dbml`·`.env.example` → **D** (RDS 일치본; `.env.example`은 track-b `DATA_SOURCE`도 자동 병합)
    * `data/` → **A**
- **결과**:
    * 전 핵심 모듈 **import OK**. `pytest` **16 passed / 2 failed**(실패 2건은 ko-agent `test_mock_graph` — mock retrieval 전제인데 통합본 `retrieval/tools.py`가 B 실검색이라 DB 접속 시도 → 코드 충돌 아니라 mock↔실 미스매치).
    * **실 RDS E2E 성공**: `/analyze`가 실 임베딩 데이터로 하이브리드 검색 → `agent_runs` 6개(market·competitor·tech·ip·bm + critic, 각 근거 5개·critic 24개 종합) → `decision: more_research` + 실제 요약 도출. **C 그래프 ↔ B 검색 ↔ D job 오케스트레이션이 실데이터로 맞물림 확인.**
    * E2E가 쓴 RDS 행은 **D 잡 라이프사이클(ideas 1 + analysis_jobs 1)뿐** — 그래프는 읽기만(분석결과 미적재). 테스트 후 **CASCADE 정리(잔존 0)**, 실데이터(`documents` 50,702건) 불변.
    * `.env` `DATABASE_URL` **비번 URL 인코딩 버그 수정** — 새 비번 특수문자가 raw로 들어가 libpq 파싱 실패 → `quote_plus` 인코딩 재조립(verify-full+cert 유지). `.env`는 gitignore라 미커밋(로컬 유지 필요).
- **범위 밖·main 가기 전 정리 항목** (전부 코드 충돌 아닌 조율):
    1. `test_mock_graph` 2건 — mock retrieval 모드 추가 또는 skip (B↔C)
    2. **agent_runs/evidence_items 실적재** — B persistence를 그래프에 배선(현재 그래프는 RDS 읽기만)
    3. **ui "신호" 칸 빔** — ko-agent `output_json`에 `signal` 키 없음 → ui 키 매핑 조율(C↔D)
- **기각**: main 직접 push(보호 브랜치 + 테스트 red + 미완 항목 → 팀 전체 전파), 통합 미루고 트랙별 따로 PR(실데이터 E2E를 못 봄).

### ADR-034 — (구현) 검색 2단계 최적화 + 노드 source_type 스코프 + 근거 0건 graceful
- **상태**: done (integration 반영). ADR-033 이후 live E2E를 돌리며 발견한 성능·정합 문제 해결.
- **맥락**: `RETRIEVAL=live`로 IP 노드가 멈춤(ui ReadTimeout). 원인 = `search/hybrid.py`가 합성점수를 **전체 테이블 `ORDER BY hybrid_score`**에 직접 걸어 HNSW 인덱스를 못 타고 `claim_limitations`(약 200만 행) 풀스캔. 또 confidence가 market/competitor/bm에서 `"low"` 하드코딩(근거 무관).
- **결정**:
    * **검색 2단계 후보생성**: `vec`(HNSW)·`kw`(GIN) CTE로 각 인덱스에서 후보 pool개씩 → 합집합(≤2×pool)에만 합성점수 계산·정렬. 풀스캔 제거. (`search_documents`·`search_claim_limitations` 둘 다)
    * **노드 source_type 스코프**: market→`seed_review`, competitor→`seed_competitor`, bm→`seed_pricing/review/competitor`. BM·시장·경쟁 근거를 특허에서 분리. retrieve()에 `source_types` 파라미터 추가(B 소유, 합의).
    * **confidence 하드코딩 제거**: `_evidence_strength`→`_confidence_from_strength`로 산정(Tech/IP와 동일). live에서 seed 근거 강도 따라 mid/high 가능.
    * **근거 0건 graceful**: 분석 노드가 근거 0건이면 run을 생략(`{"agent_runs": []}`). `AgentRun.grounded_on` min_length=1(ADR-014, 근거 없는 주장 금지)을 깨지 않으면서, **한 에이전트의 빈 근거가 잡 전체를 죽이지 않게** 함 → critic이 미검증 가설로 처리.
- **결과(중요 버그픽스)**: source_type 필터를 **후보생성 CTE 안에** 넣어야 함. seed_* 문서는 각 30건뿐이라, 전체 top-N 후보(특허 5만건 위주)를 먼저 뽑고 **나중에** 필터하면 0건이 되어 grounded_on 빈 AgentRun 검증 실패로 잡이 죽었음(간헐 실패의 원인). CTE 안으로 옮겨 해결 — 실 RDS에서 seed 5건씩 정상 반환 확인. IP 노드 ui 타임아웃 내 완료 확인.
- **기각**: 합성 ORDER BY 유지(인덱스 못 씀), `grounded_on` min 제거(근거 없는 주장 허용 = ADR-014 위배), retrieve 전체-검색 폴백(스코프 오염).

### ADR-035 — (구현) 평가 하네스 승격 — Critic ON/OFF N회 분포 (ADR-030 실행)
- **상태**: done. 실 LLM(Bedrock) 연결로 그래프가 비결정적이 되어 ADR-030 승격 조건 충족.
- **결정**: `compare_critic` 1회 → **N회 반복·분포 집계**(`compare_critic_repeated`). `change_rate`(판정 교정 비율)·`objections_mean/stdev`·`on_decision_distribution`. `evaluate(idea, n)`이 ON/OFF를 N회 돌리고 첫 회 ON으로 agent_metrics 산출.
- **결과**: "Critic이 판정 바꿈(1회 bool)" → "N회 중 교정 비율·반박 분포"로 통계적 격상. mock 검증 OK.
- **TODO 유지(사유 정정)**: `cost_usd`=`agents/llm.py`가 Bedrock 토큰 usage 캡처해야(harness 밖) / `precision_at_k`·`contradiction_coverage`=정답 라벨셋 필요.

### ADR-036 — (운영/구현) live 실행 인프라 — 임베더 캐시·HNSW 재생성·Bedrock 실연결
- **상태**: done. live+bedrock 풀 E2E가 UI까지 동작.
- **맥락/결정**:
    * **임베더 프리다운로드**: Dockerfile 빌드 시 PatentSBERTa를 미리 받아 이미지에 굽고, compose에 `hf_cache` named volume. → `live` 첫 검색에서 런타임 HF Hub 다운로드(수백 MB)로 멈추던 문제 제거. ui httpx 타임아웃 120s→600s.
    * **HNSW 인덱스 재생성**: 대량 임베딩 적재 동안 인덱스를 드롭(적재 10배 빠름)했다가, 적재 후 `idx_*_embedding`(hnsw) 재생성. ADR-034 2단계 쿼리는 인덱스가 있어야 효과 발생. → 재생성 후 IP 검색 시간 내 완료.
    * **Bedrock 실연결**: `.env`에 AWS 키 + 리전(`ap-northeast-1`) + `BEDROCK_MODEL_ID=jp.anthropic.claude-sonnet-4-6`. `AGENT_LLM_PROVIDER=bedrock`. → 실 Claude 분석 문장.
    * **`BEDROCK_MAX_TOKENS=8192`**: critic/ip의 큰 output_json이 기본 1800 토큰에서 잘려(truncation) JSON 파싱 실패 → 새 llm.py가 strict raise로 잡 사망하던 문제 완화.
- **결과**: live 검색→실 Claude 분석→DB 적재→Evidence Board까지 동작. RDS에 agent_runs/evidence_items 실적재 확인(job 단위).
- **남은 위험**: max_tokens 8192도 IP 후보 많으면 부족 가능 → 근본은 LLM이 scorecard/candidates까지 통째 재출력하는 구조를 줄이거나 graceful 폴백 필요(아래 ADR-037과 연결). 동적 IP 변경 시 RDS 보안그룹 재등록 필요.

### ADR-037 — (C, d3a5da4) mock 전면 제거 + RETRIEVAL=live 전용 강제 + 모델 sonnet 4.6 통합
- **상태**: done (integration HEAD, origin 반영). C 주도 대수술 — **우리(D)의 mock 토글 방향과 상반(설계 reversal)**.
- **맥락/결정**: C가 "live 분석에 fixed/mock 데이터 불가" 원칙으로 mock 인프라 전면 제거.
    * 삭제: `agents/mock_data.py`·`mock_repository.py`·`db_workflow.py`·`retrieval/pgvector_search.py`·`scripts/run_mock_graph.py`·`tests/test_mock_graph.py`·`agents/README.md`(775줄). 28파일 −3238줄.
    * `retrieval/tools.py`: `RETRIEVAL≠live`면 **에러**("Fixed retrieval data is not allowed") → 우리 `RETRIEVAL=mock` 토글 제거됨.
    * `agents/llm.py` 재작성: 모델 **sonnet 4.6 단일 티어**(haiku 제거), 파싱/Bedrock 실패 시 **strict RuntimeError**(graceful 폴백 없음).
    * 테스트: `test_mock_graph`→`test_live_graph`(monkeypatch로 live 경로 검증, RDS 없이 통과). pytest 30 passed.
- **결과**: import 정합 OK(잔존 mock import 0), 우리 작업(ADR-034 graceful·source_types·문구) 보존됨.
- **⚠️ 영향(팀 합의 필요)**: 오프라인/CI가 **RDS 의존**이 됨(mock 실행 불가). strict raise라 LLM 한 번의 truncation/파싱 실패로 **잡 전체 사망**(fragile) — ADR-036 max_tokens로 완화했으나 근본은 graceful 폴백(critic default_output은 코드 계산값이라 mock 아님) 또는 LLM 출력 축소 권장.
- **기각**: mock 토글 유지(C는 "live 강제"가 데이터 정합에 안전하다고 판단).

### ADR-038 — (구현) Evidence Board 출력 문구 개선 (D)
- **상태**: done (D, `c820e2b`).
