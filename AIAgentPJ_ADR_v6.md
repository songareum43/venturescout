...
> **버전**: v6 (v5에서 이어받음 — 4개 트랙 통합 머지 + 실 RDS E2E ADR-033 추가)
> **최종 갱신**: Day 0++ (C 계약 수렴·job_id 오케스트레이션 ADR-032. **+ 4개 트랙(A·B·C·D) 통합 머지 → `integration` 브랜치 + 실 RDS E2E 통과 ADR-033** — `/analyze`가 실 임베딩 데이터로 하이브리드 검색→6 agent_runs→decision 도출. `origin/integration` push, main 미반영(오류 3건 정리 후 PR).)
...
- [ ] **★integration → main 정리 3건** (ADR-033, main 가기 전) —
  1. `test_mock_graph` 2건 실패: mock retrieval 모드 추가 또는 skip (B↔C)
  2. `agent_runs`/`evidence_items` 실적재: B persistence를 그래프에 배선(현재 RDS 읽기만)
  3. ui "신호" 칸 빔: ko-agent `output_json`에 `signal` 키 없음 → 키 매핑(C↔D)
