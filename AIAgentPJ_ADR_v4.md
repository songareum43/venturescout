...
> **버전**: v4 (v3에서 이어받음 — AWS RDS 연결 전환·스키마 문서 동기화 ADR-031 추가)
> **최종 갱신**: Day 0++ (⑥ BM 노드 본문 ADR-028 / 평가 하네스 골격 ADR-029 — 3.11 기준 invoke 검증. **+ AWS RDS 연결 전환·스키마 문서 동기화 ADR-031** — verify-full 연결·init.sql↔RDS 바이트 일치 검증·track-b config.py 스모크 통과, `D` 커밋 6건. D 코드작업 사실상 종료, PR만 남음)
...
agents/graph.py       Track C — LangGraph 7노드 골격(① + ②③④⑤⑥ 병렬 + ⑦), 현재 mock·동기 / ⑥ bm_node 본문 구현됨 ★ADR-028
app/api.py            Track D — /analyze SSE 스트리밍 ★구현 완료 (ADR-023)
app/ui.py             Track D — Chainlit Evidence Board ★구현 완료 (ADR-024)
eval/harness.py       Track D — 평가 하네스(Critic ON/OFF + process 지표) ★구현 완료 (ADR-029)
...
- [x] **RDS(공용 DB)** — AWS RDS PostgreSQL 16으로 전환·스키마 문서 동기화 완료(ADR-031). ADR-005 정합 정리 끝(호스팅만 로컬→RDS, 단일 스토어 원칙 유지). `.env`는 .gitignore로 보호 확인. **잔여 open**:
  - [ ] **앱 E2E** — 앱이 실제 RDS를 읽고 쓰는 확인은 track-b(DB 코드) 머지 후 가능. 이 브랜치엔 쿼리 코드 없음(Tier A 스모크로 config 연결까지만 검증).
